...
function VisitSlotPicker({
  ...
  const allFull = VISIT_SLOTS.every((slot) => (counts[slot] ?? 0) >= VISIT_CAPACITY);

  if (allFull) {
    return (
      <div
        className="glass-sm p-5 text-center"
      ...
      >
        ...
        <h3 className="font-semibold text-slate-800 dark:text-white text-sm mb-1">{t.noSlotsTitle}</h3>
        <p className="text-slate-500 dark:text-slate-400 text-xs mb-4 max-w-xs mx-auto">{t.noSlotsMessage}</p>
        <a
          href="https://wa.me/8562057788711"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white transition-all"
          style={{ background: "rgba(37,211,102,0.88)", boxShadow: "0 2px 16px rgba(37,211,102,0.35), inset 0 1px 0 rgba(255,255,255,0.2)", border: "1px solid rgba(37,211,102,0.5)" }}
        >
...
export default function EnrollmentForm({ apiEndpoint = "/api/enroll" }: { apiEndpoint?: string } = {}) {
  ...
  const watchedGrade = watch("applying_for_grade");

  const [classCounts, setClassCounts] = useState<Record<string, number>>({});
  const [classLimit, setClassLimit] = useState(15);
  useEffect(() => {
    fetch("/api/class-slots")
      .then((r) => r.json())
      .then(({ counts, limit }) => { setClassCounts(counts ?? {}); if (limit) setClassLimit(limit); })
      .catch(() => {});
  }, []);
  useEffect(() => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="enroll-form">
      ...
      <FormSection title={t.childInfo}>
        ...
        <div>
          ...
          {watchedGrade && (classCounts[watchedGrade] ?? 0) >= classLimit && (
            <div className="glass-sm mt-3 p-5 text-center" style={{ borderColor: "rgba(251,191,36,0.45)" }}>
              <div className="w-12 h-12 bg-amber-100 dark:bg-amber-500/20 rounded-full flex items-center justify-center mx-auto mb-3">
                <svg className="w-6 h-6 text-amber-500 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                </svg>
              </div>
              <h3 className="font-semibold text-slate-800 dark:text-white text-sm mb-1">{t.classFullTitle}</h3>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-4 max-w-xs mx-auto">{t.classFullMessage}</p>
              <a
                href="https://wa.me/8562057788711"
                target="_blank"
                rel="noopener noreferrer"
