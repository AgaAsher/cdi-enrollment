...
// GET /api/admin/attendance?date=YYYY-MM-DD&class=...
export async function GET(req: NextRequest) {
  ...
  if (!date || !classLabel) return NextResponse.json({ error: "Missing params" }, { status: 400 });
...
export async function POST(req: NextRequest) {
  ...
  const { date, class_label, records } = await req.json();
  if (!date || !class_label || !Array.isArray(records)) {
