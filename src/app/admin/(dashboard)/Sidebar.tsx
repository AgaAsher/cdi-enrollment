      ...
      <Link
        ...
        className={`flex items-center justify-between px-3 py-2 rounded-xl text-[12px] font-medium transition-all duration-150 ${
          active
            ...
            : "text-[#0f1f6b]/70 hover:text-[#0f1f6b] hover:bg-white/80 dark:text-white/80 dark:hover:text-white dark:hover:bg-white/8"
        } ${depth > 0 ? "ml-3" : ""}`}
      >
        <span>{child.label}</span>
        {child.badge != null && (
          <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center ${
            active ? "bg-[#0f1f6b] text-white dark:bg-blue-500" : "bg-[#0f1f6b]/10 text-[#0f1f6b]/60 dark:bg-white/10 dark:text-white/80"
          }`}>
            {child.badge}
          </span>
        )}
  ...
  function renderSubGroup(sub: NavSubGroup) {
    ...
    return (
      <div key={sub.key} className="flex flex-col gap-0.5">
        ...
        >
          {sub.icon && (
            <span className={`shrink-0 ${isActive ? "text-[#1a3fa8] dark:text-blue-400" : "text-[#0f1f6b]/50 dark:text-white/85"}`}>
              {sub.icon}
            </span>
          )}
          <span className="flex-1 text-left">{sub.label}</span>
          <svg
            className={`w-3 h-3 transition-transform duration-200 shrink-0 ${isOpen ? "rotate-180" : ""} ${isActive ? "text-[#1a3fa8] dark:text-blue-400" : "text-[#0f1f6b]/30 dark:text-white/85"}`}
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl text-[13px] font-semibold transition-all duration-200 ${
            isActive && isOpen
              ...
              : "text-[#0f1f6b]/80 hover:bg-white/80 hover:text-[#0f1f6b] dark:text-white/85 dark:hover:bg-white/10 dark:hover:text-white"
          }`}
        >
          <span className={`shrink-0 ${isActive ? "text-[#1a3fa8] dark:text-blue-400" : "text-[#0f1f6b]/60 dark:text-white/70"}`}>
            {item.icon}
          </span>
          <span className="flex-1 text-left">{item.label}</span>
          <svg
            className={`w-3.5 h-3.5 transition-transform duration-200 ${isOpen ? "rotate-180" : ""} ${isActive ? "text-[#1a3fa8] dark:text-blue-400" : "text-[#0f1f6b]/30 dark:text-white/85"}`}
            fill="none" viewBox="0 0 24 24" stroke="currentColor"
          >
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
          </svg>
            ...
            >
              ...
              {item.badge != null && item.badge > 0 && (
                <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full min-w-[22px] text-center ${
                  isActive ? "bg-[#0f1f6b] text-white dark:bg-blue-500" : "bg-[#0f1f6b]/10 text-[#0f1f6b]/70 dark:bg-white/10 dark:text-white/85"
                }`}>
                  {item.badge}
                </span>
              )}
